...
COVER_ICON_CSV_PATH = '/data2/user_codes/wangruifeng/05MisleadingWidgets/MisleadingWidgetsDetective/Console/needcover-'
...
def displayInfoInjetor(curdbname,apkInfoTree):
    ...
    for tree_item in apkInfoTree:
        ...
        if cur_cluster_no in cluster_no_dict:
            cluster_no_dict[cur_cluster_no].append((tree_item[0]['widget'],tree_item[1]['outlier_score'],icon_name,tree_item[1]['new_cluster_no']))
        else:
            cluster_no_dict[cur_cluster_no] = [(tree_item[0]['widget'],tree_item[1]['outlier_score'],icon_name,tree_item[1]['new_cluster_no'])]
    ...
    for i in range(len(apkInfoTree)):
        ...
        cur_widget_index = cluster_no_dict[cur_cluster_no].index((tree_item[0]['widget'],tree_item[1]['outlier_score'],icon_name,tree_item[1]['new_cluster_no']))
    ...
    if os.path.exists(coverlist_csv):
        with open(coverlist_csv,'r') as f:
            ...
            #=> ['view_ba27a1df2a40cc2f63697dfc71f86c79.png', '6', 'True', '1', '10', 'True', 'True', 'True']
                #print('=>',row)
                if row[1] == '包含簇号' or row[1] == 0 or row[1] == 1:
                    continue
...
def coverIndexStatistics(apkInfoTree,cluster_no_dict,project_name,subtask_name):
    print('^^^^^^^正在统计命中信息^^^^^^^')
    # 如果这个项目的已经有了，不需要了
    if os.path.exists(COVER_ICON_CSV_PATH+project_name+'.'+subtask_name+'.csv'):
        return '' 
    ...
    for cluster_key,cluster_value in cluster_no_dict.items():
        ...
        for icon_item_index in range(len(cluster_no_dict[cluster_key])):
            ...
            if cluster_list[icon_item_index][2] in need_cover_icon: 
                ...
                cover_icon_csv.append([cluster_list[icon_item_index][2],cluster_list[icon_item_index][3],is_hit,icon_item_index,len(cluster_no_dict[cluster_key]),is_top_n_hit,is_top_20percent_hit,is_threshold_hit])
...
def album(request, projectName='MW_lle', subtask_name='spm_lle150_optics3', galleryId='',hightlightClusterId='',cluster_no=0):
    ...
    cluster_info_dict = displayInfoInjetor(DBNAME,apkInfoTree_swap)
    # 统计命中信息
    coverIndexStatistics(apkInfoTree_swap,cluster_info_dict,projectName, subtask_name)
